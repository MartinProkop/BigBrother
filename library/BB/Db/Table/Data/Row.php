...
class BB_Db_Table_Data_Row extends Zend_Db_Table_Row_Abstract {
    ...
    public function toArray($jsonExpr = true) {
        $retVal = parent::toArray();
        
        if ($jsonExpr)
            $retVal["content"] = new Zend_Json_Expr($retVal["content"]);
        else
            $retVal["content"] = Zend_Json::decode($retVal["content"], Zend_Json::TYPE_ARRAY);
